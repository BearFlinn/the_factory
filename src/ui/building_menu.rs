...
pub fn detect_building_clicks(
    ...
    if ui_interactions.iter().any(|i| matches!(i, Interaction::Pressed | Interaction::Hovered)) {
        return;
    }
    ...
    let Some(world_pos) = camera.viewport_to_world(camera_transform, cursor_pos).ok()
...
pub fn spawn_building_menu(
    ...
    for click in click_events.read() {
        ...
        if existing_menus.iter().any(|(menu, _)| menu.target_building == click.building_entity) {
            continue;
        }

        let Ok((camera, camera_transform)) = camera_q.get_single() else { continue; };
        let Ok(window) = windows.get_single() else { continue; };
        let Some(screen_pos) = camera.world_to_viewport(camera_transform, click.world_position.extend(0.0)).ok() else { continue; };

        let building_name = buildings.get(click.building_entity)
...
fn spawn_content_section(parent: &mut ChildBuilder, building_entity: Entity, content_type: ContentType) {
...
pub fn update_menu_positions(
    ...
    for (mut node, menu) in &mut menu_query {
        if let Some(screen_pos) = camera.world_to_viewport(camera_transform, menu.world_position.extend(0.0)).ok() {
        ...
        let should_update = match menu_content.content_type {
            ContentType::Status => {
                buildings_operational.get(menu_content.target_building)
                    .map(|op| simple_hash(op))
                    .map(|hash| menu_content.last_updated != Some(hash))
                    .unwrap_or(false)
            },
            ContentType::Inventory => {
                buildings_inventory.get(menu_content.target_building)
                    .map(|inv| simple_hash(inv))
                    .map(|hash| menu_content.last_updated != Some(hash))
                    .unwrap_or(false)
            },
            ContentType::Crafting => {
                buildings_crafting.get(menu_content.target_building)
                    .map(|craft| simple_hash(craft))
                    .map(|hash| menu_content.last_updated != Some(hash))
                    .unwrap_or(false)
            },
        };
        ...
        if should_update {
            ...
            commands.entity(content_entity).with_children(|parent| {
                match menu_content.content_type {
                    ContentType::Status => {
                        if let Ok(operational) = buildings_operational.get(menu_content.target_building) {
...
fn spawn_inventory_content(parent: &mut ChildBuilder, inventory: &Inventory) {
    ...
    let usage_percent = (inventory.get_total_quantity() as f32 / inventory.capacity as f32 * 100.0) as u32;
    parent.spawn((
        Text::new(format!("Capacity: {}/{} ({}%)", 
